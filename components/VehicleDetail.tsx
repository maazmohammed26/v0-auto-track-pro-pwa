...
import { ArrowLeft, Fuel, Wrench, Gauge, Edit2, Trash2, Plus, ChevronRight, Zap } from 'lucide-react'
import { toast } from 'sonner'
import { useApp } from '@/lib/context'
import type { Vehicle, FuelLog, ServiceLog } from '@/lib/store'
...
type ActiveTab = 'overview' | 'fuel' | 'service'
...
export function VehicleDetail({ vehicleId, onBack }: VehicleDetailProps) {
  const { data, deleteVehicle, deleteFuelLog, deleteServiceLog } = useApp()
  ...
  const [confirmDelete, setConfirmDelete] = useState<{ type: 'vehicle' | 'fuel' | 'service'; id?: string } | null>(null)
  ...
  return (
    <div className="flex flex-col h-screen bg-background">
          ...
          <div className="flex flex-col gap-4">
            ...
            {serviceLogs.length > 0 && (
              <div className="clay-card p-5">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">{formatDate(serviceLogs[0].date)} · ₹{serviceLogs[0].expense.toLocaleString('en-IN')}</p>
              </div>
            )}
          </div>
        )}
                ...
                <div key={log.id} className="clay-card p-4 flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <p className="text-sm font-bold text-foreground">₹{log.amount.toLocaleString('en-IN')}</p>
                      <span className="text-xs text-muted-foreground">· {log.litres.toFixed(2)} L</span>
                    </div>
                    <p className="text-xs text-muted-foreground">{formatDate(log.date)} · ₹{log.pricePerLitre}/L</p>
              ...
              serviceLogs.map(log => (
                <div key={log.id} className="clay-card p-4 flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{formatDate(log.date)} · ₹{log.expense.toLocaleString('en-IN')}</p>
                    {log.odometer && <p className="text-xs text-muted-foreground mt-0.5">{log.odometer.toLocaleString('en-IN')} km</p>}
                    {log.notes && <p className="text-xs text-muted-foreground mt-0.5 italic truncate">{log.notes}</p>}
                  </div>
                  <div className="flex gap-2 shrink-0">
      ...
      {confirmDelete && (
        <ConfirmDeleteDialog
          label={confirmDelete.type === 'vehicle' ? 'this vehicle' : confirmDelete.type === 'fuel' ? 'this fuel log' : 'this service log'}
          onConfirm={() => {
